...
export async function getEmployeesData(): Promise<EmployeesData> {
  try {
    ...
    const employees: EmployeeItem[] = empleados.map(emp => ({
      ...
      initials: (emp.Nombre || '').substring(0, 2).toUpperCase(),
      avatarUrl: `https://ui-avatars.com/api/?name=${encodeURIComponent(emp.Nombre || '')}&background=243C8F&color=fff`
    }));
...
export async function createEmployee(employeeData: any): Promise<EmployeeItem> {
  try {
    const backendData = {
      Nombre: employeeData.fullName || 'Sin nombre',
      Numero_Cedula: employeeData.idNumber || 'CED-' + Date.now(),
      ...
      Catalogo_Perfil_Usuario_idPerfil: mapProfileToId(employeeData.profile || 'Recepción'),
      Activo: true
    };
    ...
    return {
      ...
      avatarUrl: `https://ui-avatars.com/api/?name=${encodeURIComponent(response.Nombre || '')}&background=243C8F&color=fff`
    };
  } catch (error) {
