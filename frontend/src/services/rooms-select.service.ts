...
export async function getRoomsForSelect(): Promise<RoomOption[]> {
  try {
    ...
    return habitaciones.map(habitacion => ({
      label: `${habitacion.Numero_Habitacion} - ${habitacion.Tipo_Habitacion?.Nombre_Tipo || 'Sin tipo'}`,
      value: habitacion.idHabitacion.toString(),
      id: habitacion.idHabitacion,
      numero: habitacion.Numero_Habitacion,
      tipo: habitacion.Tipo_Habitacion?.Nombre_Tipo || 'Sin tipo',
      estado: habitacion.Estado_Habitacion?.Descripcion_Estado || 'Sin estado'
    }));
  } catch (error) {
